...
| Silhouette score | approx. 0.15 to 0.17 |
...
| **The Quick Spinners** | Moderate to low volume, shorter average track length |
...
7. **Model Selection**: K-Means evaluated across k = 2 to 8 using silhouette score; k = 2 excluded as a degenerate, outlier-driven split
...
12. **Production Pipeline**: feature engineering and preprocessing consolidated into a single reusable `sklearn.Pipeline`
...
`full_pipeline` bundles both the custom feature-engineering step (the `average_song_length` ratio, log transform, and percentile cap) and the fitted scalers into one object, so new data is transformed identically to the training data with a single `.transform()` call. No manual step needs to be reproduced by hand.
